        ...
        <div id='speciality' className='flex flex-col items-center gap-4 py-16 text-[#262626]'>
            ...
            <div className='flex sm:justify-center gap-4 pt-5 w-full overflow-x-auto pb-2'>
                {specialityData.map((item, index) => {
                    ...
                    return (
                        <Link to={`/doctors/${item.speciality}`} onClick={() => scrollTo(0, 0)} className='flex min-w-[180px] flex-col items-start text-xs cursor-pointer flex-shrink-0 hover:translate-y-[-10px] transition-all duration-500 rounded-2xl border border-[#E2E8FF] bg-white px-4 py-3 shadow-sm' key={index}>
                            <img className='w-16 sm:w-24 mb-2' src={item.image} alt={item.speciality} />
                            <p className='font-medium text-slate-700'>{item.speciality}</p>
                            <p className='mt-1 text-[11px] text-slate-500'>View doctors</p>
                            <div className='mt-3 flex w-full flex-col gap-1 border-t border-slate-100 pt-2'>
                                {previewDoctors.length > 0 ? (
                                    previewDoctors.map((doctor) => (
                                        <p key={doctor._id} className='truncate text-[11px] text-slate-600'>• {doctor.name}</p>
                                    ))
                                ) : (
                                    <p className='text-[11px] text-slate-400'>No doctors listed yet</p>
                                )}
                            </div>
                        </Link>
                    )
