...
const VideoCall = () => {
    ...
    useEffect(() => {
        ...
        const start = async () => {
            try {
                ...
                socket.emit('join-room', { appointmentId, role: 'patient' })
                ...
                socket.on('user-left', () => {
                    toast.info('The doctor left the call')
    ...
    const sendMessage = () => {
        ...
        socketRef.current.emit('chat-message', { appointmentId, message: messageInput, sender: 'patient' })
        setMessages(prev => [...prev, { message: messageInput, sender: 'patient', self: true }])
        setMessageInput('')
    }

    return (
        <div className='my-8'>
            <p className='mb-4 text-lg font-medium text-gray-600 dark:text-slate-300'>Video Consultation</p>
            ...
            <div className='relative bg-black rounded-lg overflow-hidden' style={{ aspectRatio: '16/9' }}>
                ...
                {chatOpen && (
                    <div className='absolute top-0 right-0 h-full w-72 bg-white dark:bg-slate-900 flex flex-col border-l dark:border-slate-700'>
                        <div className='px-3 py-2 border-b dark:border-slate-700 flex items-center justify-between'>
                            <p className='text-sm font-medium text-gray-600 dark:text-slate-300'>In-call chat</p>
            ...
            <div className='flex items-center justify-center gap-4 mt-5'>
                <button onClick={toggleMic} className={`px-4 py-2 rounded-full text-sm border ${micOn ? 'text-gray-600 dark:text-slate-300 dark:border-slate-600' : 'bg-red-500 text-white border-red-500'}`}>
                    {micOn ? 'Mute' : 'Unmute'}
                </button>
                <button onClick={toggleCam} className={`px-4 py-2 rounded-full text-sm border ${camOn ? 'text-gray-600 dark:text-slate-300 dark:border-slate-600' : 'bg-red-500 text-white border-red-500'}`}>
                    {camOn ? 'Stop Video' : 'Start Video'}
                </button>
                <button onClick={() => setChatOpen(prev => !prev)} className='px-4 py-2 rounded-full text-sm border dark:border-slate-600 text-gray-600 dark:text-slate-300'>
                    Chat
                </button>
