...
const bookAppointment = async (req, res) => {
    ...
    try {
        ...
        const { userId, docId, slotDate, slotTime, doctorData: payloadDoctorData } = req.body
        let docData = await getDoctorById(docId)

        if (!docData) {
            if (payloadDoctorData) {
                docData = {
                    ...payloadDoctorData,
                    available: true,
                    fees: payloadDoctorData.fees || 0,
                    slots_booked: {}
                }
            } else {
        ...
        } else if (docData && docData._id && String(docData._id) !== String(docId)) {
...
const paymentStripe = async (req, res) => {
    try {
        ...
        const currency = process.env.CURRENCY?.toLocaleLowerCase() || 'inr'
...
const reminderStatus = async (req, res) => {
    try {
        const { userId } = req.body
        const appointments = await appointmentModel.find({ userId, cancelled: false, isCompleted: false })
        const upcoming = appointments.filter((item) => !item.reminderSent)

        res.json({ success: true, reminderCount: upcoming.length, appointments: upcoming })
    } catch (error) {
        console.log(error)
        res.json({ success: false, message: error.message })
    }
...
const sendReminder = async (req, res) => {
    try {
        ...
        if (reminder.success) {
            await appointmentModel.findByIdAndUpdate(appointmentId, { reminderSent: true, reminderNote: 'Email reminder sent' })
            return res.json({ success: true, message: 'Reminder sent successfully' })
        }

        return res.json({ success: false, message: reminder.message })
    } catch (error) {
        console.log(error)
        res.json({ success: false, message: error.message })
    }
...
const paymentAnalytics = async (req, res) => {
    try {
        const { userId } = req.body
        const appointments = await appointmentModel.find({ userId, payment: true })
        const totalPaid = appointments.reduce((sum, item) => sum + (item.amount || 0), 0)
        const paidCount = appointments.length

        res.json({ success: true, analytics: { totalPaid, paidCount, appointments } })
    } catch (error) {
        console.log(error)
        res.json({ success: false, message: error.message })
    }
